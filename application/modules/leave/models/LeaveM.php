<?php
    ...
    class LeaveM extends CI_Model
    {
        ...
        public function f_get_leaveBal_on_maxTransaction($emp_no)
        {
            ...
            //                         AND trans_cd = (SELECT distinct trans_cd FROM td_leave_dtls WHERE emp_no = $emp_no 
            //                         AND trans_dt = (SELECT MAX(trans_dt) FROM td_leave_dtls WHERE emp_no = $emp_no) ) ");
            
$sql = $this->db->query(" SELECT  cl_bal, el_bal, ml_bal, od_bal                          FROM td_leave_dtls
              WHERE emp_no = $emp_no 
              and trans_dt=(SELECT MAX(trans_dt) FROM    td_leave_dtls 
                           WHERE emp_no = $emp_no )
                and trans_cd=(select  max(trans_cd) from  td_leave_dtls WHERE emp_no = $emp_no   and trans_dt=(SELECT MAX(trans_dt) FROM td_leave_dtls WHERE emp_no = $emp_no )) ");
