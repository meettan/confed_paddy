...
$sql="SELECT t.dist, m.district_name, SUM(t.count) count
    ...
    public function f_get_payment($pmt_bill_no,$pool_type){
        ...
        $sql = "SELECT DISTINCT t.pmt_bill_no, t.dist, t.soc_id, t.mill_id, m.block, t.pool_type,
                                t.tot_paddy, t.tot_cmr, t.trans_dt, t.rice_type, t.extra_delivery,t.paddy_butta 
    ...
    public function f_get_paids($paid_id){
        ...
        $sql = "SELECT t.payment_dt, t.paid_no, t.total_payble, t.amount,
                       t.trans_type, t.chq_no, t.chq_dt, t.bank, s.pool_type
                
                FROM td_paid_dtls t,td_paid_bill_dtls s
                                        
                WHERE t.paid_no=s.paid_no
                and  t.paid_no = $paid_id
                AND `t`.`kms_year` ='$kms_year'";
    ...
    public function f_check_bill_no($billArray, $poolType, $kms_year){
        $kms_year=$this->session->userdata('kms_yr');
        $this->db->distinct();
        $this->db->select('bill_no');
        $this->db->where('kms_yr', $kms_year);
    ...
    public function f_payble_bill_no($billArray, $poolType, $kms){
        $kms_year=$this->session->userdata('kms_yr');
        $sql = "SELECT DISTINCT tb.`pmt_bill_no` as `bill_no` 
                FROM `td_payment_bill` tb, td_bill t
                WHERE tb.con_bill_no = t.bill_no
                AND tb.`kms_year` = '$kms_year' 
                AND t.`pool_type` = '$poolType' 
                AND tb.`pmt_bill_no` IN ($billArray)";
