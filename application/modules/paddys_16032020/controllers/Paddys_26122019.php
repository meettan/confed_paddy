...
public function f_wqscdetails_report(){
    ...
    public function f_paddydeclr_report(){
        ...
        if($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            //Retriving Bill Details
            $select = array ("GROUP_CONCAT(CONCAT(bill_no)) AS `bill_no`","b.soc_name AS 'skus_nm'","substr(c.mill_name,1,length(c.mill_name)) AS 'mill_nm'","substr(soc_name,1,instr(soc_name,'S.K.U.S')-1) AS centr_nm","sum(paddy_qty) AS 'tot_qty'");
