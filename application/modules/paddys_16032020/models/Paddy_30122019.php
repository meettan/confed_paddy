...
class Paddy extends CI_Model {
    ...
    public function f_get_payments(){
        ...
        $sql = "SELECT DISTINCT `t`.`pmt_bill_no`, `md`.`district_name`, `ms`.`soc_name`, `mm`.`mill_name`, `t`.`trans_dt` 
    ...
    public function f_payment($pmt_bill_no){
        ...
        $sql = "SELECT DISTINCT t.pmt_bill_no, t.kms_year, md.district_name, ms.soc_name, mm.mill_name, ms.block,
                ...
                FROM td_payment_bill t, 
                     ...
                     md_mill mm,
                     md_district md
                WHERE t.pmt_bill_no = (SELECT DISTINCT pmt_bill_no FROM td_payment_bill WHERE con_bill_no = $pmt_bill_no)
                AND   t.soc_id = ms.sl_no
                AND   t.mill_id = mm.sl_no
    ...
    public function insert_wqsc($dis_cd,$bill_no,$pool_type,$wqsc_no,$analysis_no,$trn_dt,$no_bags,$qty,$remarks,$Unit_count)
	{
